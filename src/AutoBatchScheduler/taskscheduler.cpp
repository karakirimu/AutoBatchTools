...
void TaskScheduler::receiveTimerFinished(int value)
{
    ...
    if(value  == SchedulerCalc::EXPIRED){
        removeTask(sender()->objectName());
    }
    emit timerFinished(sender()->objectName(), value);
}
...
//QString TaskScheduler::generateRandomName(int length)
//{
