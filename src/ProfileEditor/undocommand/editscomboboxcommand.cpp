    ...
    : QUndoCommand(parent)
{
    ...
    m_oldstring = m_cache->at(m_targetindex)
                ->at(posinfo.value(XmlListGenerator::SEARCH) + 1)
                .at(1);
...
{
    QStringList list;
    list = m_cache->at(m_targetindex)->at(posinfo.value(XmlListGenerator::SEARCH) + 1);
    list.replace(1, m_oldstring);
    list.replace(3, QString::number(m_oldindex));
    m_cache->at(m_targetindex)->replace(posinfo.value(XmlListGenerator::SEARCH) + 1, list);
